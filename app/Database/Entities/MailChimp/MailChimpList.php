...
class MailChimpList extends MailChimpEntity
{
    ...
    public function getMailChimpId(): ?string
    {
     ...
     /**
     * @var \Doctrine\Common\Collections\Collection|MailChimpMember[]
     *
     * @ORM\ManyToMany(targetEntity="MailChimpMember", mappedBy="mailChimpLists")
     */
    protected $members;
    ...
    /**
     * Add member to the mailchimp list
    ...
    public function addMember(MailChimpMember $member)
    {
        ...
        $member->addMemberList($this);
    }
    /**
     * Remove member from mailchimp list
     * @param MailChimpMember $member
     */
    public function removeMember(MailChimpMember $member)
    {
        if (!$this->members->contains($member)) {
            return;
        }
        $this->members->removeElement($member);
        $member->removeMemberList($this);
    }
